...
namespace DragDropTest
{
    public partial class MainView : Form
    {
        public MainView()
        {
            ...
            dataGridView1.MouseMove += (s, e) => dataGridView1_OnMouseMove(s, e);
        ...
        /// <summary>
        /// Handles the Mouse move event for a control
        /// </summary>
        /// <param name="sender">Control from which the event was fired</param>
        /// <param name="e">MouseEventArgs</param>
        private void dataGridView1_OnMouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
        ...
        {
            var rowData = e.Data.GetData(typeof(DataGridViewSelectedRowCollection));
            if (rowData != null)
            {
                if (_dataModel2 == null)
                {
                ...
                var rows = rowData as DataGridViewSelectedRowCollection;
                foreach(var row in rows)
                {
                    var data = (row as DataGridViewRow).DataBoundItem as DataModel;
                    if (data != null)
                    {                        
                        if (!_dataModel2.Contains(data))
                        {
                            _dataModel2.Add(data);
                        }
                    }
                }
        ...
        //private Rectangle _mouseDragBox;
    }
}
